<#--
  Copied from Keycloak 26.7.3 theme keycloak.v2, with one change: the footer
  links to this application's own registration page instead of Keycloak's.
  Self registration stays off in the realm, because signing up has to assign the
  owner role, which only our backend does.
-->
    ...
    <#elseif section = "info" >
        <div id="kc-registration-container">
            <div id="kc-registration">
                <span>${msg("bmsNoAccount")} <a href="${properties.bmsRegisterUrl}">${msg("bmsDoRegister")}</a></span>
            </div>
        </div>
    </#if>
